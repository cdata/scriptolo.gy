...
Bound.prototype.update = function() {

  return this.exec(['git', '--work-tree', this.workingTree, 'fetch'])
      .then(_.bind(function() {
        return this.exec(['git', '--work-tree', this.workingTree, 'rebase'])
            .then(_.bind(function() {
              return this.exec(['git', '--work-tree', this.workingTree, 'submodule init'])
                  .then(_.bind(function() {
                    return this.exec(['git', '--work-tree', this.workingTree, 'submodule update']);
                  }, this));
            }, this));
      }, this))
};
...
Bound.prototype.readConfig = function() {
  return this.update().then(_.bind(function() {
    fs.read(this.configPath).then(function(configText) {
...
Bound.prototype.writeManifest = function() {
  return bound.resolveManifest().then(function(manifest) {
    return fs.write('bound-manifest.json', JSON.stringify(manifest));
  });
};

var bound = new Bound();
bound.writeManifest().then(function(contents) {
  console.log(new Date().toGMTString(), 'Manifest and feed generated successfully.');
}, function(error) {
  console.error(new Date().toGMTString(), error ? error.toString() : 'Failed for unknown reason.');
});
